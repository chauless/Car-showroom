...
<body class="<?php echo $themeClass; ?>">
    <form action="php/signup.php" method="post" enctype="multipart/form-data" id="form">
        ...
        <input type="text" placeholder="Karen" id="name" name="name" required pattern="^[a-zA-Z0-9]+$"
               value="<?php if (isset($_SESSION['name'])) echo htmlspecialchars($_SESSION['name']); ?>">
        <?php if (isset($_SESSION['mName'])) { echo '<p class="msg"> ' . $_SESSION['mName'] . ' </p>'; } unset($_SESSION['mName']) ?>
        <div class="msg-js">Please enter valid name</div>

        <!-- input email -->
        <label for="email">Email</label>
        <input type="email" placeholder="karen@vroom.cz" id="email" name="email" required pattern="^([a-zA-Z0-9_-]+\.)*[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)*\.[a-zA-Z]{2,6}$"
               value="<?php if (isset($_SESSION['email'])) echo htmlspecialchars($_SESSION['email']); ?>">
        <?php if (isset($_SESSION['mEmail'])) { echo '<p class="msg"> ' . $_SESSION['mEmail'] . ' </p>'; } unset($_SESSION['mEmail']) ?>
        <div class="msg-js">Please enter valid email</div>

        <!-- input password -->
        <label for="password">Password</label>
        <input type="password" id="password" name="password" required minlength="4" maxlength="30">
        <?php if (isset($_SESSION['mPassword'])) { echo '<p class="msg"> ' . $_SESSION['mPassword'] . ' </p>'; } unset($_SESSION['mPassword']) ?>
        <div class="msg-js">Please enter valid password</div>
